...
function setupScene(gltf) {
    ...
    const danceClip = THREE.AnimationClip.findByName(clips, 'dancing');
    const pushupClip = THREE.AnimationClip.findByName(clips, 'pushup');
    const danceAction = mixer.clipAction(danceClip);
    const pushupAction = mixer.clipAction(pushupClip);
    ...
    container.addEventListener('mousedown', (ev) => {
      ...
      if (intersections.length > 0) {
        if (isPushup) return;
        console.log('Pushup time!');
        isPushup = true;
        pushupAction.reset();
        pushupAction.play();
        danceAction.crossFadeTo(pushupAction, 0.3);

        setTimeout(() => {
          console.log('Dance time!');
          danceAction.reset();
          danceAction.play();
          pushupAction.crossFadeTo(danceAction, 1);
          setTimeout(() => isPushup = false, 1000);
        }, 4000)
      }
    });
